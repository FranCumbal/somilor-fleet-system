...
export default function ChecklistPage() {
  ...
  const [form, setForm] = useState({ vehiculo_id:'', chofer_id:'', turno:'dia', observaciones:'', ...initCheck() })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const [c, r] = await Promise.all([checklistAPI.list({ limit:15 }), checklistAPI.resumenHoy()])
      setChecklists(c.data); setResumen(r.data)
      setForm({ vehiculo_id:'', chofer_id:'', turno:'dia', observaciones:'', ...initCheck() })
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      {/* Formulario */}
      <Panel>
        <PanelHeader title="Nueva inspección pre-operacional">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ padding:'16px 20px', borderTop:'1px solid var(--border-soft)', display:'flex', gap:16, alignItems:'center', justifyContent:'space-between' }}>
            ...
            <button type="submit" disabled={saving || !form.vehiculo_id || !form.chofer_id}
              style={{ padding:'10px 24px', borderRadius:8, background:'var(--gold)', color:'#0E1117', border:'none', fontWeight:600, cursor:'pointer', fontSize:14, flexShrink:0, opacity: (!form.vehiculo_id || !form.chofer_id) ? 0.5 : 1 }}>
          ...
          {success && <div style={{ margin:'0 20px 16px', color: success.includes('✅') ? 'var(--green)' : 'var(--red)', fontSize:13, fontWeight:600, background: success.includes('✅') ? 'rgba(61,200,122,0.1)' : 'rgba(224,82,82,0.1)', padding:'10px 14px', borderRadius:8 }}>{success}</div>}
        </form>
      </Panel>
      ...
      <Panel>
        ...
        {loading ? <LoadingSpinner /> : checklists.length === 0 ? <EmptyState message="Sin checklists registrados" /> : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr>
                {['Fecha','Vehículo','Chofer','Turno','Resultado','Observaciones'].map(h => (
              ...
              {checklists.map(c => (
                <tr key={c.id}
                  ...
                  <td style={{ padding:'13px 20px', borderBottom:'1px solid var(--border-soft)' }}>
                    <span style={{ fontFamily:'Space Mono', fontSize:12, color:'var(--gold-light)', fontWeight:700 }}>{c.vehiculo?.codigo ?? `V-${c.vehiculo_id}`}</span>
                  </td>
