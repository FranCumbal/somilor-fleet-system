...
export default function ChecklistPage() {
  ...
  const checklistsFiltrados = checklists.filter(c => {
    ...
    return (
      ...
      (c.chofer?.nombre   || '').toLowerCase().includes(q) ||
      (c.chofer?.apellido || '').toLowerCase().includes(q) ||
      (c.turno            || '').toLowerCase().includes(q) ||
      (c.observaciones    || '').toLowerCase().includes(q) ||
      (c.aprobado === true  ? 'aprobado'  : '').includes(q) ||
      (c.aprobado === false ? 'reprobado' : '').includes(q)
    )
  })

  const totalPaginas     = Math.max(1, Math.ceil(checklistsFiltrados.length / POR_PAGINA))
  const checklistsPagina = checklistsFiltrados.slice((pagina - 1) * POR_PAGINA, pagina * POR_PAGINA)
  ...
  const eliminarChecklist = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este checklist?')) {
      try { await checklistAPI.delete(id); cargar() }
  ...
  const Paginacion = () => totalPaginas > 1 ? (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 20px', borderTop:'1px solid var(--border-soft)' }}>
      <span style={{ fontSize:12, color:'var(--text-3)' }}>
        Página {pagina} de {totalPaginas} · {checklistsFiltrados.length} registros
      </span>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20, minWidth:0, width:'100%', position:'relative' }}>
      ...
      {showForm ? (
                        ...
                        {[true, false].map(val => (
                          <button key={String(val)} type="button"
                            ...
                            style={{
                              ...
                              background: f[item.key] === val ? (val ? 'var(--green)' : 'var(--red)') : (val ? 'rgba(61,200,122,0.1)' : 'rgba(224,82,82,0.1)'),
                              color: f[item.key] === val ? (val ? '#0E1117' : '#fff') : (val ? 'var(--green)' : 'var(--red)'),
        ...
        <>
          {success && (
            <div style={{ color:success.includes('✅') ? 'var(--green)' : 'var(--red)', fontSize:13, fontWeight:600, background:success.includes('✅') ? 'rgba(61,200,122,0.1)' : 'rgba(224,82,82,0.1)', padding:'10px 14px', borderRadius:8 }}>
              {success}
            </div>
          )}
          ...
          {resumen && (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
              {[
                ...
                { label:'Reprobados',       value:resumen.reprobados,accent:'var(--red)',   data:checklistsHoy.filter(c => c.aprobado === false) },
              ].map(k => (
                <div key={k.label}
                  ...
                  <div style={{ fontSize:28, fontWeight:600, fontFamily:'Space Mono', color:k.label === 'Reprobados' && k.value > 0 ? 'var(--red)' : 'var(--text-1)' }}>{k.value}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <Panel style={{ maxWidth:'100%', overflow:'hidden' }}>
            ...
            {loading ? <LoadingSpinner /> : checklistsFiltrados.length === 0 ? <EmptyState message="Sin checklists registrados" /> : (
              <>
                        ...
                        <tr key={c.id}
                          ...
                          <td style={{ padding:'13px 20px', borderBottom:'1px solid var(--border-soft)', whiteSpace:'nowrap' }}>
                            <div style={{ fontFamily:'Space Mono', fontSize:12, color:'var(--gold-light)', fontWeight:700 }}>
                              {c.vehiculo?.placa || `V-${c.vehiculo_id}`}
                            </div>
                          ...
                          <td style={{ padding:'13px 20px', borderBottom:'1px solid var(--border-soft)', whiteSpace:'nowrap' }}>
                            <div style={{ display:'flex', gap:8 }}>
                              ...
                              <button onClick={e => { e.stopPropagation(); eliminarChecklist(c.id) }}
      ...
      {detalleActivo && (
        <div onClick={() => setDetalleActivo(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ width:'100%', maxWidth:Array.isArray(detalleActivo.data) ? '650px' : '550px', background:'var(--panel)', borderRadius:16, padding:'30px', border:'1px solid var(--border-soft)', boxShadow:'0 20px 50px rgba(0,0,0,0.5)', maxHeight:'90vh', overflowY:'auto' }}>
                  ...
                  <table style={{ width:'100%', minWidth:'500px', borderCollapse:'collapse', fontSize:13 }}>
                    ...
                    <tbody>
                      {detalleActivo.data.length === 0
                        ? <tr><td colSpan="4" style={{ padding:20, textAlign:'center' }}>No hay inspecciones en esta categoría hoy</td></tr>
                        : detalleActivo.data.map((c, i) => (
                          <tr key={i} style={{ borderBottom:'1px solid var(--border-soft)' }}>
                            ...
                            <td style={{ padding:'12px 10px' }}>
                              <div style={{ color:'#fff', fontWeight:600, fontFamily:'Space Mono' }}>{c.vehiculo?.placa}</div>
                            </td>
                            <td style={{ padding:'12px 10px' }}>{c.chofer ? `${c.chofer.nombre}` : '—'}</td>
                            <td style={{ padding:'12px 10px' }}><StatusPill status={c.aprobado ? 'aprobado' : 'reprobado'} /></td>
                          </tr>
                        ))
                      }
                    </tbody>
