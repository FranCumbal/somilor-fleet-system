...
// Agregamos 'costo' al estado inicial
...
export default function MantenimientoPage() {
  ...
  const agregarFormulario = () => {
    ...
    setError('')
    // Ya no copiamos el vehículo aquí, porque lo inyectaremos al momento de guardar
    setFormularios(prev => [...prev, { idRef: idUnico(), ...estadoInicialForm }])
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formPrincipal = formularios[0] // El maestro que tiene el vehículo, fecha y KM
      ...
      if (editandoId) {
        ...
        const promesas = formularios.map(f => {
          // A cada formulario le inyectamos los datos clave del maestro
          const payload = {
            vehiculo_id: parseInt(formPrincipal.vehiculo_id), // Heredado
            fecha_programada: formPrincipal.fecha_programada || null, // Heredado
            km_programado: formPrincipal.km_programado ? parseFloat(formPrincipal.km_programado) : null, // Heredado
            tipo: f.tipo, 
            descripcion: f.descripcion,
            taller: f.taller || null,
            costo: f.costo ? parseFloat(f.costo) : null, // Independiente
            observaciones: f.observaciones || null,
          }
          return mantenimientoAPI.create(payload)
        })
  ...
  const cerrarFormulario = () => {
    setShowForm(false)
    setEditandoId(null)
    setFormularios([{ idRef: idUnico(), ...estadoInicialForm }])
    setError('')
  }
          ...
          {formularios.map((f, index) => {
            ...
            return (
              <Panel key={f.idRef} style={{ borderLeft: esEsclavo ? '4px solid var(--gold)' : 'none' }}>
                ...
                {/* RENDERIZADO CONDICIONAL: MAESTRO VS ESCLAVO */}
                {!esEsclavo ? (
                  // --- FORMULARIO MAESTRO (Muestra todo) ---
                  <div style={{ padding:20, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16 }}>
                    ...
                    <div style={{ gridColumn:'1/3' }}>
                      <label style={{ fontSize:12, color:'var(--text-2)', display:'block', marginBottom:6 }}>Descripción del trabajo *</label>
                      <input required placeholder="Ej: Cambio de aceite 5W30" value={f.descripcion} onChange={e => updateField(f.idRef, 'descripcion', e.target.value)}
                        style={{ width:'100%', background:'var(--panel2)', border:'1px solid var(--border-soft)', borderRadius:8, padding:'9px 12px', color:'var(--text-1)', fontSize:13, outline:'none' }} />
                    </div>
                    <div>
                  ...
                  // --- FORMULARIO ESCLAVO (Súper compacto) ---
                  <div style={{ padding:20, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16, background:'rgba(255,255,255,0.01)' }}>
                    ...
                    <div>
                      <label style={{ fontSize:12, color:'var(--text-2)', display:'block', marginBottom:6 }}>Costo ($)</label>
                      <input type="number" step="any" placeholder="0.00" value={f.costo} onChange={e => updateField(f.idRef, 'costo', e.target.value)}
                        style={{ width:'100%', background:'var(--panel2)', border:'1px solid var(--border-soft)', borderRadius:8, padding:'9px 12px', color:'var(--text-1)', fontSize:13, outline:'none' }} />
                    </div>
                    <div style={{ gridColumn:'1/-1' }}>
                      <label style={{ fontSize:12, color:'var(--text-2)', display:'block', marginBottom:6 }}>Descripción del trabajo *</label>
                      <input required placeholder="Ej: Cambio de pastillas de freno" value={f.descripcion} onChange={e => updateField(f.idRef, 'descripcion', e.target.value)}
                        style={{ width:'100%', background:'var(--panel2)', border:'1px solid var(--border-soft)', borderRadius:8, padding:'9px 12px', color:'var(--text-1)', fontSize:13, outline:'none' }} />
                    </div>
                  </div>
                )}
                ...
                <tbody>
                  {mantenimientos.map(m => (
                    <tr key={m.id}
                      ...
                      <td style={{ padding:'13px 20px', fontSize:12, fontFamily:'Space Mono', color:'var(--green)', borderBottom:'1px solid var(--border-soft)' }}>
