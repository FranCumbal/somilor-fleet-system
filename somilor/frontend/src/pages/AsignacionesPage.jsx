...
export default function AsignacionesPage() {
  ...
  const cargar = () => {
    setLoading(true)
    Promise.all([
      asignacionesAPI.list({ limit: 100 }),
      vehiculosAPI.list(),
      choferesAPI.list()
    ]).then(([a, v, c]) => {
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20, minWidth: 0, width: '100%', position: 'relative' }}>
      ...
      {showForm ? (
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ padding:20, display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))', gap:16 }}>
                <div>
                  ...
                  <select value={f.vehiculo_id} onChange={e => updateField(f.idRef, 'vehiculo_id', e.target.value)} required
                    ...
                    {vehiculosAsignables.map(v => <option key={v.id} value={v.id}>{v.placa} — {v.marca} {v.modelo}</option>)}
                  </select>
                  {vehiculosAsignables.length === 0 && <span style={{fontSize: 11, color: 'var(--amber)', marginTop: 4, display: 'block'}}>No hay vehículos libres en este momento.</span>}
                </div>
                ...
                <div>
                  ...
                  <select value={f.chofer_id} onChange={e => updateField(f.idRef, 'chofer_id', e.target.value)} required
                    ...
                    {choferes.map(c => <option key={c.id} value={c.id}>{c.nombre} {c.apellido} (CI: {c.cedula})</option>)}
                  </select>
                </div>

                <div style={{ gridColumn:'1/-1' }}>
                  <label style={{ fontSize:12, color:'var(--text-2)', display:'block', marginBottom:6 }}>Observaciones de Entrega</label>
                  <textarea placeholder="Ej: Se entregan las llaves y documentos completos..." value={f.observaciones} onChange={e => updateField(f.idRef, 'observaciones', e.target.value)} rows={2}
          ...
          <div style={{ display:'flex', justifyContent:'flex-end', gap:12, marginTop:8 }}>
            ...
            <button type="submit" disabled={saving || vehiculosAsignables.length === 0} style={{ padding:'10px 24px', borderRadius:8, background:'var(--gold)', color:'#0E1117', border:'none', fontWeight:600, cursor:'pointer', fontSize:14, opacity: vehiculosAsignables.length === 0 ? 0.5 : 1 }}>
        ...
        <>
          ...
          <Panel style={{ maxWidth: '100%', overflow: 'hidden' }}>
            ...
            {loading ? <LoadingSpinner /> : asignaciones.length === 0 ? <EmptyState message="No hay asignaciones registradas" /> : (
                  ...
                  <tbody>
                    {asignaciones.map(a => (
                      <tr key={a.id} 
                        ...
                        <td style={{ padding:'14px 20px', borderBottom:'1px solid var(--border-soft)', whiteSpace: 'nowrap' }}>
                          {a.activa ? (
                            <button onClick={(e) => { e.stopPropagation(); setDetalleActivo({ tipo: 'Recepción de Vehículo', data: a, isRecepcion: true }); }} style={{ fontSize:12, padding:'6px 12px', borderRadius:6, background:'rgba(240,167,66,0.1)', color:'var(--amber)', border:'none', cursor:'pointer', fontFamily:'DM Sans', fontWeight:600 }} title="Recibir llaves y liberar vehículo">
                              📥 Recibir
                            </button>
                          ) : (
      ...
      {detalleActivo && (
        <div onClick={() => setDetalleActivo(null)} style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(10, 12, 17, 0.85)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, animation: 'fadeInModal 0.2s ease-out', padding: '20px' }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: Array.isArray(detalleActivo.data) ? '800px' : '500px', background: 'var(--panel)', borderRadius: 16, padding: '30px', border: '1px solid var(--border-soft)', boxShadow: '0 20px 50px rgba(0,0,0,0.5)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ color: 'var(--text-2)' }}>
              ...
              {/* VISTA 1A: Lista de Vehículos (Para métricas del patio) */}
              {Array.isArray(detalleActivo.data) && detalleActivo.isVehiculos && (
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', minWidth: '450px', borderCollapse: 'collapse', fontSize: 13 }}>
                    ...
                    <tbody>
                      {detalleActivo.data.length === 0 ? <tr><td colSpan="3" style={{padding:20, textAlign:'center'}}>No hay datos</td></tr> : null}
              ...
              {/* VISTA 1B: Lista de Asignaciones (Muestra Observaciones) */}
              {Array.isArray(detalleActivo.data) && !detalleActivo.isVehiculos && (
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', minWidth: '700px', borderCollapse: 'collapse', fontSize: 13 }}>
                    ...
                    <tbody>
                      {detalleActivo.data.length === 0 ? <tr><td colSpan="4" style={{padding:20, textAlign:'center'}}>No hay datos</td></tr> : null}
              ...
              {/* VISTA 2: Detalle de una sola asignación (Lectura) */}
              {!Array.isArray(detalleActivo.data) && !detalleActivo.isRecepcion && (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                    ...
                    {detalleActivo.data.activa ? <StatusPill status="en_proceso" overrideText="En Ruta" /> : <StatusPill status="completado" overrideText="Finalizado" />}
                  </div>
              ...
              {/* VISTA 3: Formulario de Confirmación de Recepción (Terminar) */}
              {!Array.isArray(detalleActivo.data) && detalleActivo.isRecepcion && (
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                    ...
                    <button 
                      onClick={() => confirmarRecepcion(detalleActivo.data.id)}
                      disabled={saving}
